...
$TailnetGatewayLogPath=Join-Path $LogsRoot 'Training\tailnet-gateway.log'
$GoVersion='1.27.1'
...
switch($Command){
    ...
    Invoke-Checked $bridge @('auth','--state',$state,'--hostname',$hostname) $AssetsRoot

    if(Test-Path -LiteralPath $TailnetGatewayPidPath){
        $oldPid=0
        [void][int]::TryParse((Get-Content -LiteralPath $TailnetGatewayPidPath -Raw).Trim(),[ref]$oldPid)
        if($oldPid -gt 0 -and (Get-Process -Id $oldPid -ErrorAction SilentlyContinue)){ return }
        Remove-Item -LiteralPath $TailnetGatewayPidPath -Force -ErrorAction SilentlyContinue
    }
    ...
    $p=Start-Process -FilePath $bridge -ArgumentList $argList -WorkingDirectory $AssetsRoot -RedirectStandardOutput $TailnetGatewayLogPath -RedirectStandardError $TailnetGatewayLogPath -WindowStyle Hidden -PassThru
    Start-Sleep -Milliseconds 750
    if($p.HasExited){ throw "Embedded tailnet gateway exited during startup. Check $TailnetGatewayLogPath" }
    $p.Id | Set-Content -LiteralPath $TailnetGatewayPidPath -NoNewline -Encoding ASCII
    Write-Host "Embedded tailnet gateway online as $hostname on tailnet TCP $tailnetPort (PID $($p.Id))."
}
