...
### BUG-026 — Initial placement overwrites authored squad formation offsets
**Status:** Open  
**Location:** `Scripts/Levels/LevelConstructor.cs`, `SpawnShipsAndSquads()` / `PositionSquads()`; `Scripts/Levels/Squad.cs`, `SetStartingPosition()`; `Scripts/Levels/Squad.Movement.cs`, `SetOffsets()`  
**Description:** `Ship.Setup()` initially receives each persisted `SquadShip.Offset`, but `PositionSquads()` then calls `SetStartingPosition()`, which scales those offsets for Queen, Bumblebee, wide, and ultra-wide ships to produce temporary spawn positions. Immediately afterward it calls `SetOffsets()`, which recalculates every ship's `OffsetFromCenter` from those scaled world positions and thereby replaces the authored formation. Subsequent squad movement uses the altered offsets rather than the formation the player saved, so simply starting a level can permanently change the runtime shape/spacing of an otherwise valid saved formation.