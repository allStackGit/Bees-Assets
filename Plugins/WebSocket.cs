...
namespace NativeWebSocket
{
  ...
  public class WebSocket : IWebSocket {
    ...
    public class WebSocket : IWebSocket
    {
        ...
        {
            //UnityEngine.Debug.Log($"Dispatch message queue running Thread: {Thread.CurrentThread.ManagedThreadId}");
            ...
            {
                //UnityEngine.Debug.Log($"Locking incoming messages and copying Thread: {Thread.CurrentThread.ManagedThreadId}");
                messageListCopy = new List<byte[]>(m_MessageList);
                m_MessageList.Clear();
            }
            //UnityEngine.Debug.Log($"Lock is ended Thread: {Thread.CurrentThread.ManagedThreadId}");
            var len = messageListCopy.Count;
            for (int i = 0; i < len; i++)
            {
                //UnityEngine.Debug.Log($"Sending OnMessage invokation Thread: {Thread.CurrentThread.ManagedThreadId}");
