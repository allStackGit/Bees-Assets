...
namespace Bees.Tests.EditMode
{
    ...
    {
        [Test]
        public void RemoveShipAvoidsPoolOnlyRegistryScansAndResetClearsThem()
        {
            ...
            StringAssert.DoesNotContain("statuses.RemoveAll", method);
            StringAssert.DoesNotContain("spotted.RemoveAll", method);
            StringAssert.Contains("ShipsToRelease.Add(ship)", method);

            string statePath = Path.Combine(Application.dataPath, "Scripts", "Levels", "GameState.cs");
            string state = File.ReadAllText(statePath);
            StringAssert.Contains("SpottedShips = new[] { new List<SpottedShip>(), new List<SpottedShip>() }", state);
            StringAssert.Contains("ShipDamageStatuses[0].Clear()", state);
            StringAssert.Contains("ShipDamageStatuses[1].Clear()", state);
            StringAssert.Contains("ShipsToRelease.Clear()", state);
        }
