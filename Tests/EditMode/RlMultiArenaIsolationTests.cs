...
namespace Bees.Tests.EditMode
{
    ...
    public class RlMultiArenaIsolationTests
    {
        ...
        {
            string multiArena = File.ReadAllText(Path.Combine(
                Application.dataPath,
                "Scripts",
                "Scenes",
                "RlOneVsOneMultiArenaBootstrap.cs"));
            string durability = File.ReadAllText(Path.Combine(
                Application.dataPath,
                "Scripts",
                "Scenes",
                "RlOneVsOneTrainingDurabilityGuard.cs"));

            Assert.That(multiArena, Does.Contain("IReadOnlyList<Level> levels = _stage.Levels;"));
            Assert.That(multiArena, Does.Contain("for (int levelIndex = 1; levelIndex < levels.Count; levelIndex++)"),
                "The multi-arena bootstrap must explicitly process every non-primary Level.");
            Assert.That(multiArena, Does.Contain("RlOneVsOneTrainingDurabilityGuard.ApplyTrainingDurability(ship);"));
            Assert.That(durability, Does.Contain("internal const float TrainingHealthFraction = RlOneVsOneTrainingOptions.DefaultHealthRatio;"));
            Assert.That(durability, Does.Contain("CalculateTrainingHealth(originalHealth, RlOneVsOneTrainingBootstrap.CurrentHealthRatio)"),
                "Primary and secondary arenas must both resolve durability from the configured training health ratio.");
        }
