...
internal static class RlEpisodeShipIdentity
{
    ...
    private sealed class EpisodeIdentityState
    {
        ...
        {
            // For a power-of-two modulus, every odd multiplier is coprime with the modulus, making
            // (ordinal * multiplier + offset) a true permutation of the complete code space.
    ...
    private static System.Random GetIdentityRandom(Level level)
    {
        ...
        {
            int seed = RlOneVsOneScenarioSeed.Create(level, RlOneVsOneScenarioSeed.IdentityStreamSalt);
    ...
    private static void HandleEpisodeEnded(Level level, RlOneVsOneEpisodeCoordinator.EpisodeResult result)
    {
        if (level != null)
        {
            // Keep the per-Level random stream alive so the next episode receives a new permutation
            // without making one arena's reset timing perturb another arena's sequence.
