    ...
    public class LevelInputManager
    {
        ...
        public void LoadHotKeySettings()
        {
            ...
            _hotKeys.ForEach((hotKey) =>
            {
                ...
                {
                    case "Match Speed":
                        hotKey.SetAction(() => Stage.Menus.ActionBox.MatchSpeed());
                        break;
                    case "Attack on Sight":
                        hotKey.SetAction(() => Stage.Menus.ActionBox.AttackOnSight());
                        break;
                    case "Cease Fire":
                        hotKey.SetAction(() => Stage.Menus.ActionBox.CeaseFire());
                        break;
                    case "Patrol":
                        hotKey.SetAction(() => Stage.Menus.ActionBox.Patrol());
                        break;
                    case "Guard":
                        hotKey.SetAction(() => Stage.Menus.ActionBox.Guard());
                        break;
                    case "Chase":
                        hotKey.SetAction(() => Stage.Menus.ActionBox.Chase());
                        break;
                    case "Hold":
                        hotKey.SetAction(() => Stage.Menus.ActionBox.Hold());
                        break;
                    case "Detonate":
                        hotKey.SetAction(() => Stage.Menus.ActionBox.Detonate());
                        break;
                    case "Charge":
                        hotKey.SetAction(() => Stage.Menus.ActionBox.Charge());
                        break;
                    case "Drop Beacon":
                        hotKey.SetAction(() => Stage.Menus.ActionBox.DropBeacon());
                        break;
                    case var _ when ConfigData.ShootingStrategyNames.Contains(hotKey.Name):
                        hotKey.SetAction(() => Stage.Menus.ActionBox.SetShootingStrategy(hotKey.Name));
                        break;
                    case var _ when hotKey.Name.StartsWith("Select Squad #"):
                        hotKey.SetAction(() => SelectSquadByNumber(int.Parse(hotKey.Name.Substring(hotKey.Name.Length - 1))));
                        break;
                    case "Open Menu":
                        ScaledTimer resetKeyTimer = new ScaledTimer();
                        hotKey.SetAction(() =>
                        {
                            ...
                            if (resetKeyTimer.IsCanceled)
                            {
                                resetKeyTimer.Reuse(.5f, () => hotKey.ManuallySetInputRelease(true));
                                Level.AddTimer(resetKeyTimer);
                            }
                        });
                        break;
                    case "Show Ranges":
                    ...
                    case "Manual Fire":
                        hotKey.SetAction(() =>
                        {
                            if (!IsFiringManually) ManualFire();
                        });
                        hotKey.SetReleaseAction(() =>
                        {
                            if (IsFiringManually) ManualFire();
                    ...
                    case "Show Ranges + Manual Fire":
                        ...
                        hotKey.SetReleaseAction(() =>
                        {
                            ...
                            if (IsShowingRanges && !ConfigData.GetUserSettingsData().FindKey("Show Ranges").HasInput()) ShowRanges();
                            if (IsFiringManually && !ConfigData.GetUserSettingsData().FindKey("Manual Fire").HasInput()) ManualFire();
                        });
                        break;
                    case "Toggle Mini Map":
                        hotKey.SetAction(() => Stage.Menus.ToggleMiniMapDisplay());
                        break;
                    case "Move Camera Up":
                        hotKey.SetAction(() => ScrollUp());
                        break;
                    case "Move Camera Right":
                        hotKey.SetAction(() => ScrollRight());
                        break;
                    case "Move Camera Down":
                        hotKey.SetAction(() => ScrollDown());
                        break;
                    case "Move Camera Left":
                        hotKey.SetAction(() => ScrollLeft());
                        break;
                }
            });
        }
        ...
        public void Update()
        {
            ...
            {
                if (Timers[_update_i].Update()) Timers.RemoveAt(_update_i);
            }
        }
        ...
        private float _checkActions_mouseScrollSpeed;
        ...
        private void CheckInputs()
        {
            ...
            if (Input.GetKey(KeyCode.LeftShift)) _leftShift = true;
            else if (Input.GetKey(KeyCode.LeftControl)) _leftControl = true;
            ...
            if (Input.GetMouseButtonDown(LeftClick))
            {
                ...
                if (_leftMouseClicked) _leftMouseDoubleClicked = true;
                _leftMouseClicked = true;
                _leftMouseClickedTime = Time.realtimeSinceStartup;
            }
            else if (Input.GetMouseButtonUp(LeftClick))
            {
                if (EventSystem.IsPointerOverGameObject() && !_isLeftMouseDragging) return;
            ...
            {
                if (EventSystem.IsPointerOverGameObject()) return;
                if (Mathf.Abs(Input.GetAxis("Mouse X")) > .5f || Mathf.Abs(Input.GetAxis("Mouse Y")) > .5f) _isLeftMouseDragging = true;
            }
        ...
        {
            if (Stage.Camera.orthographicSize == Level.Map.MaxZoom) Stage.Camera.orthographicSize = Stage.DefaultZoom;
            else Stage.Camera.orthographicSize = Level.Map.MaxZoom;
        }

        private void SetRightMouseDownLongEnoughForDragging()
        {
            if (_isRightMouseDownPrior) _isRightMouseDragging = true;
        }

        private bool HasDragMoveSquadsInput()
        {
            if (!_isDragMovementBlockedByTimer && !EventSystem.IsPointerOverGameObject() && _isRightMouseDragging &&
        ...
        private void UnblockDragMovement() => _isDragMovementBlockedByTimer = false;
        ...
        private void CheckActions()
        {
            ...
            foreach (HotKey hotKey in _hotKeys) hotKey.CheckInput();
            if (HasPauseInput()) Debug.Break();
            ...
            if (!Level.State.IsPaused)
            {
                ...
                else if (HasMoveSquadsInput()) MoveSquads(Stage.Camera.ScreenToWorldPoint(Input.mousePosition));
                else if (_leftMouseButtonUp)
                {
                    if (_isLeftMouseDragging && !_selectingPatrolArea) Selector.SelectShipsInBox();
                    else if (!LeftClickAction()) Selector.SelectShipsInBox();
                    _isLeftMouseDragging = false;
                    Selector.Deactivate();
                }
                if (_isLeftMouseDragging) Selector.DrawSelectionBox(_mouseDownPosition, _mousePosition);

                if (_scrollPositive)
                {
                    if (_leftShift) ScrollLeft();
                    else if (_leftControl) ScrollUp();
                    else ZoomIn();
                }
                else if (_scrollNegative)
                {
                    if (_leftShift) ScrollRight();
                    else if (_leftControl) ScrollDown();
                    else ZoomOut();
                }

                if (Stage.UseMouseScrolling || ConfigData.UserProgressData.UseMouseScrolling)
                {
                    if (_mouseAtLeftEdge) ScrollLeft(_checkActions_mouseScrollSpeed);
                    else if (_mouseAtRightEdge) ScrollRight(_checkActions_mouseScrollSpeed);
                    if (_mouseAtTopEdge) ScrollUp(_checkActions_mouseScrollSpeed);
                    else if (_mouseAtBottomEdge) ScrollDown(_checkActions_mouseScrollSpeed);
                }
            }
        }
        ...
        private void MoveSquads(Vector2 targetPosition)
        {
            ...
            if (_moveSquads_selectedSquads.Count > 0)
            {
                ...
                Level.RecordSimulationInput("user-move", $"{string.Join(",", _moveSquads_selectedSquads.Select(squad => squad.ItemId))}|{_moveSquads_localized.x:R}|{_moveSquads_localized.y:R}");
            }
                ...
                for (_checkClickCollision_i = 0; _checkClickCollision_i < _checkClickCollision_hits.Length; _checkClickCollision_i++)
                {
                    ...
                    if (_checkClickCollision_hit.collider != null)
                    {
                        if (_checkClickCollision_hit.collider.CompareTag("Ship"))
                            _clickedShip = _checkClickCollision_hit.collider.gameObject.GetComponent<Ship>();
                        else if (_checkClickCollision_hit.collider.CompareTag("Beehive Heal Collider"))
                            _clickedShip = _checkClickCollision_hit.collider.transform.parent.gameObject.GetComponent<Ship>();
                    }
                }
            }
        }
            ...
            if (!Stage.Menus.HoveringOverMiniMapButton)
            {
                ...
                for (_checkForMiniMapNavigation_j = 0; _checkForMiniMapNavigation_j < _minimapNavigationResults.Count; _checkForMiniMapNavigation_j++)
                {
                    ...
                    if (_checkForMiniMapNavigation_hit.gameObject.name == "Camera Collider")
                    {
                        ...
                        if (mouseButton == RightClick) MoveSquads(_viewportWorldPoint);
                        else
                        {
        ...
        private bool LeftClickAction()
        {
            ...
            if (!CheckForSelectingSquad() && !CheckForSelectingPatrolArea()) return false;
            return true;
        }
        ...
        {
            if (scrollSpeed == 0) scrollSpeed = Stage.ScrollSpeed;
        ...
        private void ZoomIn()
        {
            _zoomIn_difference = -Stage.ZoomSpeed;
            if (Stage.Camera.orthographicSize + _zoomIn_difference < Level.Map.MinZoom)
            {
                _zoomIn_difference = Level.Map.MinZoom - Stage.Camera.orthographicSize;
            }
        ...
        private void ZoomOut()
        {
            ...
            {
                _zoomOut_difference = Level.Map.MaxZoom - Stage.Camera.orthographicSize;
            }
