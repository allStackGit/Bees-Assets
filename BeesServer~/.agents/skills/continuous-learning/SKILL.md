...
description: Closed-loop BeesServer learning that improves routing and reduces rediscovery. Verify candidate lessons, promote only durable high-ROI knowledge, and prefer compression or route repair over documentation growth.
---
...
- an important runtime path, request/data/transaction boundary, protocol/schema relationship, or safety rule was expensive to rediscover;
- current code/tests/schema disprove maintained guidance;
...
Before promotion, verify the lesson from current source/tests/schema and the Unity client contract when applicable. Then ask whether keeping it will reduce future work more than it increases mandatory or likely reading.
...
- prevents a likely repeated transaction/security/protocol/data-integrity mistake;
...
- runtime/data/request architecture -> `docs/DEVELOPMENT_MEMORY.md`;
- schema/data identity/persistence flow -> `docs/DATABASE_MODEL.md`;
...
Over repeated tasks, agents should reach the right BeesServer-specific source/schema/evidence with fewer reads/searches and repeat fewer mistakes. If accumulated learning makes a small task slower, the learning system must be compressed or rerouted.