...
| distributed training control, leases, trainer registry, build distribution, operator commands, run rollover, training-log archival | `docs/TRAINING_CONTROL.md`; root `bees.ps1` | `trainingControl.js`, `trainingControlCli.js`; client `Training/bees_training_control.py`, `bees_training_worker_agent.py`, `bees_run_lifecycle.py`, `bees_archive_training_run.py`; Unity `Editor/BeesCommandLineBuild.cs` | desired-state revision, staged compatible/incompatible rollout, run compatibility key, run-scoped logs/checkpoints, dedicated fail-closed lease, full-game inference fallback, SHA-256 artifacts, `B:\\Bees` server/build/start/stop/status flow |
