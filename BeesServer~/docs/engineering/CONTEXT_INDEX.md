...
| distributed training control, leases, trainer registry, build distribution, operator commands | `docs/TRAINING_CONTROL.md`; root `bees.ps1` | `trainingControl.js`, `trainingControlCli.js`; client `Training/bees_training_control.py`, `bees_training_worker_agent.py`; Unity `Editor/BeesCommandLineBuild.cs` | desired-state revision, role-specific canonical artifacts, dedicated fail-closed lease, full-game inference fallback, SHA-256 artifacts, `B:\\Bees` build/start/stop/status flow |
