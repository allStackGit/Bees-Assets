...
`remoteSshTarget` is no longer used as an Internet/LAN destination by the generated tailnet launchers. If it contains `user@host`, only the `user` portion is used to select the Windows SSH account; otherwise Bees uses the current learner Windows username. The host side is always reached through the embedded private tailnet. SSH key authentication remains recommended for unattended reconnects; password authentication can still be used interactively.
