...
| WebGL IL2CPP request comparer, `MethodAccessException`, `IEqualityComparer<ServerRequest>` | regressions REG-008; `StandingRequestSet` | `ServerRequestSet`, `Socket._waitableRequests`, `ConfigData.__PastServerRequests`, `Level.Reset` | request `Hash` is transport identity; avoid `ServerRequest`-keyed generic hash comparers; validate `GetSettings` request/response + resend path in browser |
...
| responsive UI, Mac, aspect ratio, white strip, Main Menu spacing, mission objective panel, squad-tab wrapping | regressions REG-001..REG-003; system map → Screen-space UI | `ResponsiveScreenLayoutGuard`, `GameHudLayoutGuard`, `RootCanvasCompatibilityGuard`, `Scenes/Main Menu.unity`, `Scenes/Space.unity` | inset panel ownership; blank mission status; 1–10 squad tabs; geometry tests + rendered platform validation |
