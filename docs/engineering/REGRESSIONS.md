...
**Permanent protection:** `GameHudLayoutGuard` now makes the legacy Squad Tabs root fill the live canvas and derives the row's left padding from the rendered right edge of the active scoreboard plus a small gap; when no scoreboard is visible it uses a small top-left inset. Scoreboard, ordinary Game Speed, action-box and mini-map edge placement use a 10-unit rendering inset so their visible borders do not touch the exact canvas boundary. `RootCanvasCompatibilityGuard` no longer owns Squad Tabs and instead repairs viewport ownership only; for a full-screen `VerticalLayoutGroup` with one dominant fixed-height body and fixed footer/tool rows, it assigns viewport-height surplus to the dominant body so the footer remains at the real bottom. `GameHudLayoutGuardTests` behaviorally protects scoreboard-relative tab padding and edge inset geometry; `RootCanvasCompatibilityGuardTests` behaviorally protects taller-viewport body/footer fill and rejects a second squad-tab owner.  
**Verification:** compile/import in Unity `6000.5.4f1`, run `BeesFoundation`, then repeat the supplied Mac checks: gameplay scoreboard and Game Speed must be fully inset, squad numbers must start immediately to the scoreboard's right (or top-left inset when no scoreboard exists), selected-squad action box and mini map must be fully visible, Level Intro bottom controls must remain visible, and Squad Maker/other menu screens must have no white bottom strip. Repeat at a normal 16:9 size to ensure the reference layout remains unchanged apart from the intentional small edge inset. Rendered Mac validation is still required for this revision; EditMode geometry alone cannot prove final platform rendering.  
**Invariant/knowledge:** `docs/engineering/INVARIANTS.md` and `docs/engineering/SYSTEM_MAP.md` now record scoreboard-relative squad-tab ownership, visible edge inset semantics, and dominant-body surplus handling for viewport-level vertical layouts.
