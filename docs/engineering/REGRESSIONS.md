...
**Root cause:** distinct edge contracts were conflated. Navigation controls such as Back/Continue/Skip benefit from a small rendering margin, but the gameplay HUD is intentionally authored flush to the canvas edges; applying the same 10-unit inset to gameplay controls changed the intended layout. Separately, the Squad Tabs and scoreboard are sibling UI branches in the live `Space` scene, so calculating scoreboard bounds as though the scoreboard were a descendant of the stretched Squad Tabs root produced an incorrect left offset. The viewport/body/footer issue remains a separate root-layout ownership concern handled by `RootCanvasCompatibilityGuard`.  
**Permanent protection:** `GameHudLayoutGuard` uses zero gameplay edge margin and explicitly pins the scoreboard and ordinary Game Speed to the top corners, the selected-squad action box to bottom-left, and the mini map to bottom-right. The Squad Tabs root still fills the live canvas, but its first-tab offset converts the scoreboard's right edge through world space into Squad Tabs local space before adding the tab gap; when the scoreboard is hidden, the row starts at the top-left canvas edge. `RootCanvasCompatibilityGuard` remains the separate owner of viewport-level body/footer repair and the 15-unit margin for explicit navigation controls. `GameHudLayoutGuardTests` behaviorally protects sibling scoreboard/tab geometry, zero hidden-scoreboard fallback, flush corner pinning and the zero gameplay margin; `RootCanvasCompatibilityGuardTests` continues to protect root-layout and navigation-margin behavior.  
...
**Invariant/knowledge:** `docs/engineering/INVARIANTS.md` and `docs/engineering/SYSTEM_MAP.md` distinguish flush gameplay HUD edges from navigation-control margins, record sibling-space scoreboard/tab geometry, and preserve the separate viewport-layout ownership contract.
