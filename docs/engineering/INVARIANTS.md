...
- Children driven by a Unity `LayoutGroup` remain under that layout group's geometry ownership. Generic responsive repair must not rewrite their anchors, offsets, positions, or sizes; make the owning screen container responsive instead. A viewport-level `LayoutGroup` owner must itself fill the actual canvas so its footer/top-row children are laid out against the real display rather than the 1366x768 reference rectangle.
...
- Gameplay HUD edge contracts are explicit: squad-number tabs belong at the actual top-left after the final Unity layout pass, the selected-squad action box must remain fully visible at bottom-left, and the mini map must remain fully visible at bottom-right.
