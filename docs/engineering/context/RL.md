...
| player / Hive Mind demonstration capture and human imitation | `RlGameplayDemonstrationAgent`, `Training/bees_continual_train.py` | opt-in `--rl-record-demonstrations`; shared-ABI ML-Agents `.demo` files are separated under `Application.persistentDataPath/RlDemonstrations/Human` and `/HiveMind`; `--continual-human-demo-dir` snapshots trusted human demos and injects configurable behavioral cloning into a generated runtime YAML; live-RL-controlled ships are excluded; one-shot special actions still require authoritative pre-event capture; authenticated public upload/central native-demo ingestion is not implemented yet |
