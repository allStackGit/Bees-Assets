...
| public RL demonstration upload | `RlDemonstrationUploader`, `SteamWebApiAuth`; BeesServer `rlDemonstrationUploads.js` / `security.js` | background upload owns a dedicated authenticated WSS connection rather than gameplay `Socket` request/reconnect state; desktop Production only; three request types (`rl-demo-begin`, `rl-demo-chunk`, `rl-demo-complete`) are Steam identity-bound and quarantine-only; 16 MiB limit applies to demo+manifest together |
| WebGL browser WebSocket | `Plugins/WebSocket.jslib`, `Plugins/WebSocket.cs`, `WebSocketFactory` | browser callback to managed bridge; Development WebGL evidence; public RL demo upload is intentionally unavailable because Steam Web API authentication is unavailable there |
