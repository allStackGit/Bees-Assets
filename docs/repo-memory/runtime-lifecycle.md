...
- Ordinary dead `Ship` wrappers are retained in `GameState.ShipsToRelease` during the live level and are not returned to their pools until the reset/teardown boundary. `ResetState()` runs runtime cleanup/release and then clears the level registries before those wrappers can be reused. Do not add global or whole-registry scans to synchronous ship/squad casualty paths solely to protect against later pool reuse; clean state immediately only when live semantics require it, otherwise clear it at reset or prune it during an existing traversal.
- Keep normal combat casualty teardown proportional to the dying object and its direct ownership edges. Whole-registry scans (all damage records, spotted records, every Hive Mind observer set, global pending requests, or copy/clear/rebuild visibility sets) compound badly in large battles and previously all ran synchronously on death. Move stale-state pruning to reset or an existing reader traversal unless live semantics require immediate removal.
- A death regression intended to cover gameplay casualties must exercise `Kill(..., endKill: false)` or the lethal damage entry point. `EndKill()` / `endKill: true` is teardown/retreat behavior and deliberately skips explosion, combat statistics, range cleanup, and other casualty-only work; it is not sufficient evidence for an in-battle death freeze.
...
- Hive Mind observer ownership is live state: remove the departing observer's dictionary entry when the ship dies so it stops contributing vision immediately. Do not scan every other observer set from `RemoveShip()` merely to erase the dead ship as a seen target; `GetShipsVisibleToHiveMind()` already traverses those sets and prunes dead targets there, keeping casualty teardown bounded.
...
- `ShipDamageStatus` and `SpottedShip` can retain direct dead `Ship` wrappers during the remainder of a live level. Because ordinary ship wrappers are not reused until reset, do not scan these registries from every `RemoveShip()` solely for pooling safety; the reset boundary clears them before wrapper reuse. Lethal combat code must also avoid recreating a damage-status record after killing/removing the target.
