...
- The playable campaign completion boundary is the scripted `CampaignMissionCatalog`, not `Configuration.TotalLevels`. The global version-5 configuration row in the current database dump advertises `TotalLevels = 10` while server-backed campaign data and this client implement IDs 0-11. Because those sources are inconsistent, progress at 12 must be treated as beta-campaign completion rather than deriving the boundary from `TotalLevels`.
