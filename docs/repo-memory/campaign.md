...
- The playable campaign completion boundary is the scripted `CampaignMissionCatalog`, not `Configuration.TotalLevels`. The active server configuration currently advertises 32 total/future levels while this client and server-backed campaign data implement IDs 0-11; progress at 12 must be treated as beta-campaign completion rather than passed to `LoadLevel()`.
