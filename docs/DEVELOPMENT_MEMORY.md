...
- `RangeCollider` owns weapon-range enter/exit bookkeeping. Ship exits remove `Weapon.ShipsWithinRange`, set `Weapon.HasCachedChanged`, and remove the reverse `WeaponsThatHaveUsWithinRange` relation while the exiting ship is still alive. Map-object exits update `GameState.PlayerVisibleMapObjects`. Enter/exit tests should use multiple simultaneously tracked objects so stale callback fields (`_colliderEnter` vs `_colliderExit`) cannot hide copy/paste ownership bugs.
...
- `BeesServer/siServerDev.js` is an executable monolith: it defines `Common`, `Database`, `SocketRequest`, `SocketConnection`, and `Server`, exports none of them, and ends by instantiating `Server`. Its production classes can still be tested safely by loading the source into Node `vm`, replacing only the final startup statement with exports, and stubbing `mysql2`, HTTP, WebSocket, and timers.
- The production `Database.query` borrows a pool connection, releases it after either SQL success or SQL failure, and rejects pool-acquisition failures. `handleDisconnect` rebuilds the pool on `PROTOCOL_CONNECTION_LOST`/inactivity errors.
- `SocketConnection` deduplicates concurrent work using `server.pendingRequests` keyed by request `Hash` before appending to `server.queue`; WebSocket close removes the connection from `server.connections`.
- The production server currently has no begin/commit/rollback transaction API. Transaction qualification is therefore an implementation/testability gap, not an untested existing contract; add a real transaction boundary before writing rollback/commit tests.
