...
- `ConfigData.Version` is part of the database contract. `ServerSettings.Fetch()` sends the exact version for `configuration`, `starting-settings`, and `ship-stats`; BeesServer performs an exact `(user/name/version)` query and does not safely fall back. Changing `ConfigData.Version` therefore requires matching settings rows whose JSON shape satisfies the current Unity parsers.
- Current source has `ConfigData.Version = 5`, while the supplied August 2026 database dump has only `ship-stats` v2 plus `configuration`/`starting-settings` v2/v4, and those old payloads omit fields required by current `Configuration`, `StartingSettings`, and `ShipStats`. Treat this as a data/version migration issue; do not solve it with blind fallback.
- Hive Mind matchup IDs are deterministic unsigned xxHash64 values produced by BeesServer and kept as decimal strings in Node/MySQL `BIGINT UNSIGNED`. Unity gameplay should not depend on signed `long` debug MatchupId fields; use string/explicit unsigned handling if matchup IDs become functional client data.
...
- `stored_user_data` is opaque JSON keyed logically by `(userId, filename)`. Missing ordinary user rows are expected: `Socket.HandleUserDataResponse` generates/writes the appropriate default data and retries. Current server code forces `campaign_levels_data` and `challenge_levels_data` to synthetic shared user ID `2`; never use ID 2 for test users.
