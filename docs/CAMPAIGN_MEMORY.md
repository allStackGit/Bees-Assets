...
- The authored Minesweeper prefab currently resolves all 30 Fire Tanks to one serialized `TargetObstacle`. Because `CanisterBomb.Kill()` breaks exactly its `TargetObstacle`, the active mission repairs duplicated/missing links at setup by pairing each Fire Tank with the nearest unused obstacle. If a future prefab contains 30 valid unique authored links, the repair leaves them untouched.
