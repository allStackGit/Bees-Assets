...
The current frozen policy contract is ABI v20: behavior `BeesRL1v1`, 7614 vector observations, 16 continuous actions, and discrete branches `2x5,5`. The vector contains 7593 perception values followed by episode progress and 20 reserved tail values. The healing special action is weapon-exclusive: choosing healing suppresses all five weapon-fire branches for that decision. v19 models and demonstrations are not ABI-compatible with v20 and must not be relabeled or silently reused as v20 data.
...
python Training\bees_continual_train.py Training\rl_1v1_config.yaml `
  ...
  --continual-root="F:\RLDemo\BeesContinualV20" `
  --continual-game-build="2026.09.12" `
  --continual-human-demo-dir="C:\path\to\RlDemonstrations\PolicyV20\Human"
```
...
python Training\bees_continual_native_demo.py `
  ...
  --model-id="bees-rl-v20-<deployed-model-id>" `
  --game-build="2026.09.12" `
  "C:\path\to\RlDemonstrations\PolicyV20\Human\human-s0.demo"
```
...
BeesServer accepts only `Human` uploads through `rl-demo-begin`, `rl-demo-chunk`, and `rl-demo-complete`. Sessions are bound to the authenticated Steam user and connection, rate/size limited, hash checked, and quarantined with `readyForTraining: false`. The server's accepted capture policy is part of the cross-repository frozen ABI contract; for the current build it is ABI v20 / 7614 observations with the exact `RlPolicySchema.Signature`.
...
The miner and geometry suggester decode the frozen ABI v20 compact-field indices directly, including scrambled scalar ship identities, current map/position fields, and the first visible enemy slot. They require the exact current observation size and fail closed on another ABI.
...
python Training\bees_continual_adversarial_evaluate.py `
  ...
  --baseline="bees-rl-v20-<baseline-id>" `
  --opponent="bees-rl-v20-<opponent-id>" `
  --env="F:\RLDemo\Bees RL Training" `
...
Configure the corresponding BeesServer deployment so `BEES_RL_MODEL_DISTRIBUTION_DIR` points at that same distribution root (for example `D:\BeesRlModels`). If neither that environment variable nor an explicitly injected distribution root is configured, the authenticated `rl-model-current` / `rl-model-chunk` route intentionally fails closed with model distribution disabled rather than serving from an implicit path.
...
Omitting `--target` restores the registry's recorded previous champion. Supply `--target=bees-rl-v20-...` to restore a particular historical champion or to retry release reconciliation after a partial rollback failure. Hot-bundle reactivation only uses retained immutable bundles that still exactly match the newly current deployment.
