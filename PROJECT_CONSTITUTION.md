...
7. **Network and persistence contracts are cross-component contracts.** Unity request/response shapes, versioning, request hashes/deduplication, game/level ownership, reconnect state, and storage routing must remain compatible with the server contract under `BeesServer~/`. Do not change one side speculatively.
